...
- 角色条目按角色名建二级子目录（如 `角色/林小雨/基础信息.yaml`）
- MVU 变量相关文件放在 `变量/` 下
- EJS 文件放在 `EJS/` 下
...
{
  "contents": [
    { "content": "---\n<character_basic character=\"林小雨\">" },
    { "file": "世界书/角色/林小雨/基础信息.yaml" },
...
1. 写入文件 `世界书/角色/林小雨/基础信息.yaml`
2. 执行命令：

```bash
node scripts/tavern-cards-forge.mjs patch {project} '[{"op":"add","path":"/entryManifest/角色/林小雨_基础信息","value":{"part":"basic","scope":"specific","abstract":"林小雨的基础信息：17岁女高中生，乐队贝斯手","contents":[{"content":"---\n<character_basic character=\"林小雨\">"},{"file":"世界书/角色/林小雨/基础信息.yaml"},{"content":"</character_basic>"}],"keywords":["林小雨"]}}]'
```
...
| 复杂度 | 处理时机 | 操作 | 参考文档 |
|-------|---------|------|---------|
| 条目显隐 | 条目注册时 | `contents` 首片段嵌入 `@@if`，不编辑内容文件 | `references/ejs/guide.md` |
