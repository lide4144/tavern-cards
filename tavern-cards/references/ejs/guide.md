...
## Preprocessing 条目与变量注册

条件中引用的变量名必须在 `EJS预处理` 条目中通过 `define()` 注册。多条目共享变量名时，也集中放在 Preprocessing 条目中注册。

`getvar` 路径必须带 `stat_data.` 前缀，使用点分隔：`stat_data.角色.好感度`。

从规划文档的 `ejs.preprocessing` 获取变量映射。构建 EJS预处理 条目（`@@preprocessing` + `define()` 语句）。

- entryManifest 中不存在 EJS预处理 类型条目 → 创建，写入 `世界书/EJS/EJS预处理.txt`，运行 patch 注册
...
| 装饰器 | 作用 |
|--------|------|
| `@@preprocessing` | 世界书处理前执行（用于 Preprocessing + 动态激活🟢） |
...
3. 多条目共享变量名，推荐创建 Preprocessing 条目
4. 装饰器之间不能有空行
5. `@@preprocessing` 不能和 `@@generate_before/after` 同时使用

## EJS 收尾检查

MVU 变量完成后执行（SKILL.md 主流程第 6 步）。

1. 收集 EJS 条件中引用的变量名（从规划文档的 `ejs.entries[].condition` 和 `ejs.preprocessing` 获取），与 schema.ts 中已定义的变量对照。未定义的变量需要返回 MVU 流程补全。
2. 遍历所有已处理的 EJS 条目，收集实际使用的变量名，与 EJS预处理 中已注册的变量对照，有缺漏则追加。
