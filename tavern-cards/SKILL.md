...
帮助用户创建和编辑 SillyTavern 角色卡与世界书的内容和配置。覆盖角色人设编写、世界观构建、MVU 变量系统、EJS 动态方案、条目配置、开场白创作等全流程。聚焦于内容创作和流程编排，涉及 schema.ts 和 EJS 模板但不涉及通用脚本逻辑。

## 设计原则

角色卡本质上一套专门针对角色扮演的 AI 提示词工程。其中蕴含一对矛盾：角色扮演需要丰富的细节设定来维持人设一致性，而 AI 提示词则要求简洁、准确的表述以节省 token。因此采用三种策略解决这一矛盾：
...
1. **需求对齐：项目属性**：收集项目属性（名称、形式、MVU/EJS 需求、typeLists 调整等）→ `references/requirements.md`
...
6. **创建 EJS预处理 条目 + EJS 收尾检查**（如需 EJS）→ `references/ejs/guide.md`，EJS预处理 条目使用 `@@generate_before` 装饰器
...
9. **打包输出** → `references/packaging.md`
   - 执行 `node scripts/tavern-cards-forge.mjs pack {project}`
   - 如使用 MVU，完成状态栏前端开发（见 packaging.md）
...
- **tavern-cards-forge CLI**：离线打包/解包/配置工具，完整命令用法与数据模型见 `references/manual.md`。
